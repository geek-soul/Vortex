...
typedef struct x264_param_t
{
    ...
    //added by fan 2013.10.30
    //add a new parameter for interleaving reference. i.e. The picture 0,2,4,6,8... is a reference sequence, while picture 1,3,5,7,9... is another reference sequence
    //when a single picture is corrupted when transmission, there's always a sequence can be decoded correctly.
    int b_yy_interleave_mode;
    
    //added by Yue
    int b_yy_lowrate_mode;
    
    //added by zhangwei
    int b_yy_decreasing_iframe;

    //added by yanleju
    void *custom_data;
} x264_param_t;

typedef struct x264_switch_param_t {
	int b_yy_lowrate_mode;
	int b_yy_interleave_mode;
	int i_keyint_max;
	int i_keyint_min;
