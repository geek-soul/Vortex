...
void x264_param_default( x264_param_t *param )
{
    ...
    param->b_yy_interleave_mode = 0;
    //added by Yue
    param->b_yy_lowrate_mode = 0;
    //added by zhangwei
    param->b_yy_decreasing_iframe = 0;
}
...
#define OPT2(STR0, STR1) else if( !strcmp( name, STR0 ) || !strcmp( name, STR1 ) )
    ...
    //added by fan 2012.12.18
    OPT("yy-interleave-mode")
    	p->b_yy_interleave_mode = atoi(value);
    //added by Yue
    OPT("yy-lowrate-mode")
    	p->b_yy_lowrate_mode = atoi(value);    
    //added by zhangwei    	
    OPT("yy-decreasing-iframe")
        p->b_yy_decreasing_iframe = atoi(value);
    else
        return X264_PARAM_BAD_NAME;
#undef OPT
#undef OPT2
