                                    ...
                                    int i_idr_pic_id, int i_frame, int i_qp )
{
    ...
    //added by fan 2012.12.18
    if(h->param.b_yy_interleave_mode && h->param.i_frame_reference>1)
    {
        if(h->fenc->i_type == X264_TYPE_P && h->i_ref[0]>1)
        {
        	if(sh->b_ref_pic_list_reordering[0] == 0)	//if there is reordering operation, do nothing to avoid conflicts
        	{
        		//check the relationship of frame numbers
        		int max_frame_num = 1 << sps->i_log2_max_frame_num;
        		int diff1 = (i_frame - h->fref[0][0]->i_frame_num + max_frame_num) % max_frame_num;
        		int diff2 = (h->fref[0][0]->i_frame_num - h->fref[0][1]->i_frame_num + max_frame_num) % max_frame_num;
        		if(diff1 == 1 && diff2 == 1)
        		{
        			//set reordering syntax
        			sh->b_ref_pic_list_reordering[0] = 1;
        			sh->ref_pic_list_order[0][0].idc = 0;
        			sh->ref_pic_list_order[0][0].arg = 1;		//use current_frame_num -2 as ref0
        			sh->ref_pic_list_order[0][1].idc = 3;		//stop reordering for ref1
        			sh->ref_pic_list_order[0][1].arg = 0;

        			//switch reference order
        			h->fref[0][2] = h->fref[0][1];
        			h->fref[0][1] = h->fref[0][0];
        			h->fref[0][0] = h->fref[0][2];
        		}
        	}
        }
    }
...
static void x264_slice_header_write( bs_t *s, x264_slice_header_t *sh, int i_nal_ref_idc )
{
    ...
    if( sh->i_type != SLICE_TYPE_I )
    {
        ...
        if( sh->b_ref_pic_list_reordering[0] )
        {
            ...
            {
            	//added by fan 2012.12.18. Stop when the idc is 3, because we may only set reordering for ref0
            	if(sh->ref_pic_list_order[0][i].idc >=3)
            		break;
...
x264_t *x264_encoder_open( x264_param_t *param )
{
    ...
    if (param->custom_data) {
    	h->constants_source = (x264_t*)param->custom_data;
    }
    ...
    /* init CPU functions */
    if (h->constants_source) {
    	memcpy(&h->predict_16x16, &h->constants_source->predict_16x16, (uint8_t*)&h->constants_source->lookahead - (uint8_t*)&h->constants_source->predict_16x16);
    ...
    for( qp = X264_MIN( h->param.rc.i_qp_min, QP_MAX_SPEC ); qp <= QP_MAX; qp++ ) //h->param.rc.i_qp_max
...
//FIXED ME!!! >>>> This reinit func must be used very carefully since it has many assumptions
int x264_encoder_reinit(x264_t *h, x264_param_t *param ) {	
	int reinit_flag = 0;
	if(h->param.b_yy_lowrate_mode != param->b_yy_lowrate_mode || h->param.b_yy_interleave_mode != param->b_yy_interleave_mode) {
    	reinit_flag = 0;
    }
    else {
    	reinit_flag = 1;
    }
    reinit_flag = reinit_flag && (h->param.rc.i_rc_method == X264_RC_ABR) && (h->param.i_bframe == 0);
	h->param.b_yy_lowrate_mode = param->b_yy_lowrate_mode;
    h->param.b_yy_interleave_mode = param->b_yy_interleave_mode;
    h->param.i_keyint_max = param->i_keyint_max;
    h->param.i_keyint_min = param->i_keyint_min;
    ...
    h->param.i_fps_num = param->i_fps_num;
    h->param.i_fps_den = param->i_fps_den;
    h->param.rc.i_qp_max = param->rc.i_qp_max;
	...
	h->thread[0]->lookahead->i_last_keyframe = - h->param.i_keyint_max;
    
    mbcmp_init( h );
    chroma_dsp_init( h );
    
	x264_set_reinitflag(h, reinit_flag);
    x264_ratecontrol_reinit( h );
    return 1;
fail:
    x264_free( h );
    return 0;
...
void x264_add_vbvbuffer(x264_t *h, unsigned int extratime) {
	double extraframe = (double)extratime * (double)h->param.i_fps_num / 1000.0;
//	h->param.rc.buffer_fill_final += (uint64_t) (extraframe * (double)(h->sps->vui.hrd.i_bit_rate_unscaled * h->sps->vui.i_num_units_in_tick * 2)); //h->fenc->i_cpb_duration
}
    ...
    *pp_nal = NULL;
    ...
    if( pic_in != NULL )
    {
        ...
        if (fenc->i_type == X264_TYPE_IDR) {
        	// forced I frame, should be very small to get a rapid restart.
        	h->last_I_count = 0;
        }
        if( fenc->i_frame == 0 )
    ...
    else if( h->fenc->i_type == X264_TYPE_P )
    {
        i_nal_type    = NAL_SLICE;
        i_nal_ref_idc = NAL_PRIORITY_HIGH; /* Not completely true but for now it is (as all I/P are kept as ref)*/        
        //added by fan 2012.12.19
        if(h->param.b_yy_interleave_mode && h->param.i_frame_reference>1)
        {
        	if(h->i_frame_num > 1)					//when interleave mode, all P frames (except first P) set as 00 00 01 21
        	{
