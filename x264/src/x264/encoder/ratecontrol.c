...
void x264_set_reinitflag(x264_t *h, int reinitflag) {
	h->rc->reinit_flag = reinitflag;
}

void x264_ratecontrol_reinit( x264_t *h )
{
    x264_ratecontrol_t *rc;
	
    x264_emms();
	double oldfbit, fbitratio;
    rc = h->rc;
    
    if(rc->reinit_flag) {
    	if(rc->pred[SLICE_TYPE_P].coeff	> 5.0 || rc->pred[SLICE_TYPE_P].coeff < 0.8 || rc->pred[SLICE_TYPE_P].offset > 1.0 || rc->pred[SLICE_TYPE_I].coeff	> 5.0 || rc->pred[SLICE_TYPE_I].coeff < 0.8 || rc->pred[SLICE_TYPE_I].offset > 1.0) {
    		rc->reinit_flag = 0;	
    	}
    	if(rc->row_preds[SLICE_TYPE_P][0].coeff > 6.0 || rc->row_preds[SLICE_TYPE_P][0].coeff < 0.7 || rc->row_preds[SLICE_TYPE_P][0].offset > 10.0 || rc->row_preds[SLICE_TYPE_I][0].coeff > 6.0 || rc->row_preds[SLICE_TYPE_I][0].coeff < 0.7 || rc->row_preds[SLICE_TYPE_I][0].offset > 10.0) {
    		rc->reinit_flag = 0;		
    	}
    	if(rc->row_preds[SLICE_TYPE_P][1].coeff > 1.0 || rc->row_preds[SLICE_TYPE_P][1].coeff < 0.06 || rc->row_preds[SLICE_TYPE_P][1].offset > 10.0) {
    		rc->reinit_flag = 0;	
    	}
    	if(rc->fps < 3 && rc->accum_p_norm < 6) {
    		rc->reinit_flag = 0;		
    	}
    }
    if(rc->reinit_flag) {
		oldfbit = rc->bitrate / rc->fps;
	}
    /* FIXME: use integers */
    if( h->param.i_fps_num > 0 && h->param.i_fps_den > 0 )
        rc->fps = (float) h->param.i_fps_num / h->param.i_fps_den;
    ...
    rc->cbr_decay = 1.0;
    if(rc->reinit_flag) {
		fbitratio = rc->bitrate / rc->fps / oldfbit;
	}
	//reset value
	rc->filler_bits_sum = 0;
	rc->short_term_cplxsum = 0;
	rc->short_term_cplxcount = 0;
...
#define ABR_INIT_QP (( h->param.rc.i_rc_method == X264_RC_CRF ? h->param.rc.f_rf_constant : 24 ) + QP_BD_OFFSET)
		if(rc->reinit_flag) {
			rc->accum_p_qp = qscale2qp(qp2qscale(rc->accum_p_qp / rc->accum_p_norm) / fbitratio) * 0.01;
        	rc->accum_p_norm = .01;
//        	double tmpwanted_bits_window = 1.0 * rc->bitrate / rc->fps;
//        	rc->cplxr_sum = (rc->cplxr_sum / (rc->wanted_bits_window)) * tmpwanted_bits_window / fbitratio;
//        	rc->wanted_bits_window = tmpwanted_bits_window;
		}
		else {
	        rc->accum_p_norm = .01;
    	    rc->accum_p_qp = ABR_INIT_QP * rc->accum_p_norm;
...
int x264_ratecontrol_mb( x264_t *h, int bits )
{
    ...
    if( y < h->i_threadslice_end-1 )
    {
		...
		if((int)(h->rc->fps)>1&&h->i_frame_num<=(int)(h->rc->fps)&&h->i_frame<(int)(h->rc->fps)&&h->param.i_threads==1)
		{
			double avgwantbits = (float)(rc->pbwant)/(float)(rc->pbcount);
			if(avgwantbits<h->fenc->f_duration * rc->bitrate*0.1)
			{
				avgwantbits = h->fenc->f_duration * rc->bitrate*0.1;
			}
			float pAddThrold = 0.9*avgwantbits;
			float pSubThrold = 0.8*avgwantbits;
			while( rc->qpm < qp_max+2
	               &&( (((b1 > rc->frame_size_planned + rc_tol) ||
	                   (rc->buffer_fill - b1 < buffer_left_planned * 0.5f) ||
	                   (b1 > rc->frame_size_planned && rc->qpm < rc->qp_novbv)) 
				   && ((h->sh.i_type==SLICE_TYPE_P&&b1>pAddThrold)||(h->sh.i_type==SLICE_TYPE_I)))
				   ||((h->sh.i_type==SLICE_TYPE_P&&b1>1.8*pAddThrold))
				   )
					)

	        {
             ...
             &&((h->sh.i_type==SLICE_TYPE_P&&b1<1.8*pSubThrold)||(h->sh.i_type==SLICE_TYPE_I))
             )
			 ||(h->sh.i_type==SLICE_TYPE_P&&b1<pSubThrold&&(rc->qpm > qp_min-3)) 
             )
		     {
		          rc->qpm -= step_size;
		          b1 = predict_row_size_sum( h, y, rc->qpm ) + size_of_other_slices;
...
#ifdef __DECREASING_I_FRAME__
void x264_ratecontrol_set_mb_qp( x264_t *h,float new_qp_offset)
{
    x264_emms();
    if( h->param.rc.i_aq_mode )
    {
        if( h->fdec->b_kept_as_ref )
            h->fenc->f_qp_offset[h->mb.i_mb_xy] += new_qp_offset;
        else
            h->fenc->f_qp_offset_aq[h->mb.i_mb_xy] += new_qp_offset;
    }
}
#endif //__DECREASING_I_FRAME__
...
int x264_ratecontrol_end( x264_t *h, int bits, int *filler )
{
    ...
    if( rc->b_abr )
    {
		...
		//printf("frameno=%d,i_frame_num=%d,fps=%d,thno=%d\n",h->i_frame,h->i_frame_num,(int)(h->rc->fps),h->param.i_threads);
		if((int)(h->rc->fps)>1&&h->i_frame_num<=(int)(h->rc->fps)&&h->i_frame<(int)(h->rc->fps)&&h->param.i_threads==1)
		{
			if(h->sh.i_type == SLICE_TYPE_I)
			{
				...
				else if(h->rc->fps<=6.0)
				{
					Ifps = 0.5*h->rc->fps;
				}
				else if(h->rc->fps<=7.0)
				{
					Ifps = 0.4*h->rc->fps;
				}
				else
				...
				rc->pbfps = (h->rc->fps - Ifps)/(h->rc->fps-1);
				rc->wanted_bits_window += Ifps*h->fenc->f_duration * rc->bitrate;
				rc->pbwant = (h->fenc->f_duration * rc->bitrate*h->rc->fps) -bits;
				rc->pbcount = (int)(h->rc->fps)-1;
			}
			else
			{
				if(rc->pbfps<=0.0)
				...
				rc->wanted_bits_window += rc->pbfps*h->fenc->f_duration * rc->bitrate;
				rc->pbwant = rc->pbwant -bits;
				rc->pbcount = rc->pbcount-1;
			}
		}
		else
		{
...
static double clip_qscale( x264_t *h, int pict_type, double q )
{
    ...
    if( rcc->b_vbv && rcc->last_satd > 0 )
    {
        ...
        else
        {
			...
			if(pict_type == SLICE_TYPE_I && rcc->fps > 5) {
				double max_fill_factor2 = (double)((int)rcc->fps >> 1);
				max_fill_factor2 = max_fill_factor2 > 5.0 ? 5.0 : max_fill_factor2;
				double tmp1 = max_fill_factor * bits;
				double tmp2 = max_fill_factor2 * (bits - 3.5 * rcc->buffer_rate);
				tmp1 = tmp1 > tmp2 ? tmp1 : tmp2;
				if( tmp1 > rcc->buffer_fill )
    	            qf = x264_clip3f( rcc->buffer_fill/tmp1, 0.2, 1.0 );
			}
			else {
    ...
    else
    {
        ...
        else /* 1pass ABR */
        {
            ...
            if( pict_type == SLICE_TYPE_I && h->param.i_keyint_max > 1
                ...
                && (rcc->last_non_b_pict_type != SLICE_TYPE_I || rcc->reinit_flag) )
            {
                float q2 = qp2qscale( rcc->accum_p_qp / rcc->accum_p_norm );
                q2 /= fabs( h->param.rc.f_ip_factor );
                if(rcc->reinit_flag) {
                	q = q > q2 ? q : q2;	
                }
                else {
...
#ifdef __DECREASING_I_FRAME__
        if (h->param.b_yy_decreasing_iframe) {
        	if (pict_type == SLICE_TYPE_I && h->param.i_keyint_max > 1) {
        		int original_size = predict_size( &rcc->pred[h->sh.i_type], q, rcc->last_satd);
        		int cur_size;
        		int ideal_size = rcc->bitrate / rcc->fps * 3 / 2;
        		int cut_ratio = 0.5;

        		if (h->last_I_count < h->param.i_keyint_max * 1 / 4) {
        			cut_ratio = 0;
        			ideal_size = rcc->bitrate / rcc->fps;
        		}
        		while((cur_size = predict_size( &rcc->pred[h->sh.i_type], q, rcc->last_satd)) > ideal_size && cur_size >= original_size * cut_ratio) {
        			q *= 1.01f;
        		}
        		h->last_I_count = 0;
        	}
